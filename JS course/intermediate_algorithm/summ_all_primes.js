/*
A prime number is a whole number greater than 1 with exactly two
divisors: 1 and itself. For example, 2 is a prime number because it is only
divisible by 1 and 2. In contrast, 4 is not prime since it is divisible
by 1, 2 and 4.

Rewrite sumPrimes so it returns the sum of all prime numbers that are
less than or equal to num.
 */

function sumPrimes(num) {
  let sum = 0
  let primes = [2]
  for (let number = 3; number <= num; number++) {
    // console.log('Checking number', number)
    let numberIsPrime = true
    for (let prime of primes) {
      // console.log('Checking number', number, 'with prime', prime)
      if (number % prime === 0) {
        // console.log('Redundant number', number, 'prime', prime)
        numberIsPrime = false
        break
      }
    }
    if (numberIsPrime && !primes.includes(number)) {
      console.log('Adding number', number)
      primes.push(number)
    }
  }
  // console.log('Primes found:',primes)
  return primes.reduce(
      (accum, item) => accum + item
  )
}
console.log(
  sumPrimes(999999)
);